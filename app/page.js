...
export default function Home() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const chartRes = await fetch(`/api/history?id_sensor=${selectedSensor}&limit=15`);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-2">
        <h1 className="text-white text-3xl font-bold tracking-tight text-slate-900">Dashboard Monitoring</h1>
      </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
              <div className="flex items-center gap-3">
                <h2 className="font-bold text-slate-800 text-xl">Grafik Ketinggian Air</h2>
              </div>
              <select
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-bold text-slate-800 text-lg">Status Sensor</h2>
            </div>
            <div className="space-y-4">
              {data.map((d) => (
                <div key={d.id_sensor} className="flex items-center justify-between p-4 rounded-xl bg-slate-50 border border-slate-200 transition-all hover:shadow-md hover:border-blue-100 group">
                  <div className="flex flex-col">
                    <span className="font-semibold text-slate-800 group-hover:text-blue-600 transition-colors">{d.nama_sensor}</span>
                    <span className="text-[10px] text-slate-400 font-medium">
                      Data terkini: {new Date(d.timestamp).toLocaleString("id-ID", {
                        dateStyle: "medium",
                        timeStyle: "short"
                      })}
                    </span>
                  ...
                  <div className="flex flex-col items-end">
                    <span className="text-xl font-bold text-slate-900">{d.tinggi_air} <span className="text-sm font-normal text-slate-400">cm</span></span>
                    <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-md mt-1 ${
                      d.status === "normal" ? "bg-emerald-100 text-emerald-700" :
                      d.status === "siaga" ? "bg-amber-100 text-amber-700" :
          ...
          <div className="bg-white border-2 border-blue-50 rounded-2xl p-6 shadow-sm">
            <h3 className="font-bold text-slate-800 text-lg mb-2">Pusat Bantuan</h3>
            <p className="text-slate-500 text-sm mb-5 leading-relaxed">Segera hubungi BPBD Pamekasan jika terjadi keadaan darurat banjir di sekitar Anda.</p>
            <a 
              href="tel:112"
